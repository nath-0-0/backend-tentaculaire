...
import { itemSchema } from './item';
  ...
  export interface ILend {
    idUserBorrow: string,  // si l'utilisateur desactive son compte, on met ancien user
    Item :{item_id :string,title:string} // pour l'historique, celui ci peut apparait donc encore si l'objet n'existe plus
    dateFrom: number,
    dateEnd : number,
    ratingState: Boolean, //Yes or No
    ratingDelay: Boolean, //Yes or No
    status: string //  ask askOk Out refused history (confirmIn) --> 7 jours après History
  }
  ...
  // schema definition
  const lendSchema = new Schema<ILendDoc>({
    iduserborrower:{
